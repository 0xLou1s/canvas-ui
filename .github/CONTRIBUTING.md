...
- `src/lib/` is the library itself. Each component has one folder containing the framework wrappers (React `*.tsx`, Solid `*.solid.tsx`, Vue `*.vue`, Svelte `*.svelte`, and vanilla `*.ts`) and a shared `*Vanilla.ts` engine where the canvas/WebGL logic lives.
