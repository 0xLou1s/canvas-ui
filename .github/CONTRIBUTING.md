...
- `src/lib/` is the library itself. Each component has one folder containing the framework wrappers (`*.tsx`, `*.vue`, `*.svelte`, vanilla `*.ts`) and a shared `*Vanilla.ts` engine where the canvas/WebGL logic lives.
